...
bool pertenece(int x, LinkedList xs) {
    ...
    bool respuesta = current(ixs) == x; // lo guardo en variable porque si no lo pierdo.
    DisposeIterator(ixs);
    return respuesta;
}
...
int main(){
    ...
    cout << pertenece(4, xs) << endl;
    cout << pertenece(10, xs) << endl;
    Snoc(5,xs); Snoc(5,xs); Snoc(2,xs);
    imprimirLista(xs);
    cout << apariciones(5,xs) << endl;
    cout << minimo(xs) << endl;
